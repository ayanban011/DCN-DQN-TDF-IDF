...
##
##ead On 2015-11-02
## By xyongle: email:xyongle@{163.com,ncu.edu.cn}
##  Data operate including JPEG encode and decode
##  function: processing the data
###############################################################################################################

    ## Red by xyongle@163.com in 2015-11-15
    ## JPEG processing one by one batch
    ##  function: processint the JPEG using c++ dp.convnet.libmodel.decodeJpeg
class JPEGBatchLoaderThread(Thread):
    ...
    ##
    ## read by xyongle@163.com in 2015-11-15
    ## load data according to rawdics: rawdics is the data file content and do not any process
    ## the data are processing by  c++ dp.convnet.libmodel.decodeJpeg
    ##
    @staticmethod
    def load_jpeg_batch(rawdics, dp, label_offset):
        ...
        #print 'jpeg_name ' * 10
        if 'filenames' in rawdics[0].keys():
          jpeg_names = list(it.chain.from_iterable(rd['filenames'] for rd in rawdics))
        else:
          jpeg_names = [str(v[0]) for v in labels]
          print '=============================================================================='
        #print 'jpeg_name=%s' % (jpeg_names)
        # jpeg_name_mat = n.array(jpeg_names).reshape(len(jpeg_names[0]), len(jpeg_names))
        #print jpeg_names[0:10]
        ...
        #################################################################
        ##      comment: if add the laster in the return ,the other part may be not need to modify.
        ## because this add jpeg_names, so th convnet.py:line 175 modify:
        ## from : if max(d.dtype != n.single for d in batch_data[2]):
        ## to:    if max(d.dtype != n.single for d in batch_data[3]):
        rtn = {'data': img_mat[:nc_total * dp.data_mult,:],
                'labvec': lab_vec[:nc_total * dp.data_mult,:],
                'labmat': lab_mat[:nc_total * dp.data_mult,:],
                'jpeg_names':jpeg_names}
        #print '####################################################'
        #print len(jpeg_names)
        return rtn
...
##
## Read by xyongle@163.com in 2015-11-15
## add the noise to data according to the parameters: --color_noise
...
#############################################################################################################
##
## data process core class
##LabeledDataProvider
##              -->ImageDataProvider
##              -->CIFARDataProvider
##              -->MNISTDataProvider
##              -->DummyConvNetLogRegDataProvider
##
class ImageDataProvider(LabeledDataProvider):
    ...
    def showimg(self, img):
        ...
        pl.show()
    ## obtain the value of num_vis in batch_meta
    ## the value is the size of image
    ...
    ################################################################################################################################
    ##
    ## load data one by one according to the curr_epoch and curr_batchnum
    ## The data will minus the data-mean-crop if it is needed
    ##
    def get_next_batch(self):
        ...
        #print 'convdata.py return four elements ' * 3 
        return epoch, batchnum, [self.data[self.d_idx]['data'].T, self.data[self.d_idx]['labvec'].T, self.data[self.d_idx]['labmat'].T], self.data[self.d_idx]['jpeg_names']
...
#################################################################################################################
##
##  the same with ImageDataProvider
##
class CIFARDataProvider(LabeledDataProvider):
    def __init__(self, data_dir, batch_range=None, init_epoch=1, init_batchnum=None, dp_params=None, test=False, num_colors = 3):
        LabeledDataProvider.__init__(self, data_dir, batch_range, init_epoch, init_batchnum, dp_params, test)
        self.img_size = 32 
...
class MNISTDataProvider(LabeledDataProvider):
    def __init__(self, data_dir, batch_range=None, init_epoch=1, init_batchnum=None, dp_params=None, test=False, num_colors = 3):
        ...
        self.data_dic = []
        for i in batch_range:
            print 'self.get_data_file_name(i)    ' * 10
            self.data_dic += [unpickle(self.get_data_file_name(i))]
            self.data_dic[-1]["labels"] = n.require(self.data_dic[-1]['labels'], dtype=n.single)
            self.data_dic[-1]["labels"] = n.require(n.tile(self.data_dic[-1]["labels"].reshape((1, n.prod(self.data_dic[-1]["labels"].shape))), (1, self.data_mult)), requirements='C')
            self.data_dic[-1]['data'] = n.require(self.data_dic[-1]['data'] - self.scalar_mean, dtype=n.single, requirements='C')
