...
##################################################################################
##  Note 2015-11-07 by xyongle@163.com
## This modle is the core of python, The function include:
##  (1)collect data:image data, user parameters, layer configure, checkpoint and so on.
##  (2) Transfer the data to C++ ConvNet, Then the C++ ConvNet will process the data for compute, including make a network,
##            training the Network, testing the networks, asy between CPU and GPU, the state saveing,
##            printing the information of runing(Traning process, Training error and  test error) and so on.
##  (3) The information may be include six classes:
##      A:Connect with Dataprovider: Dataprovider process the data for train if the parameters train = true \
##                                                                 for test if the parameter train = false
##      B: Connect with optionsparser
##      C: Connect with LayerParse
##      D: Connect with checkpoint, such as save or read data
##      E: Connect with create or print methods
##      F: Connect with C++ code: main about: ConvNet
##
##################################################################################
...
##
## GPU Model interface
##  __init__: model_name: ConvNet, model name
##
##################################################################################
class IGPUModel:
    def __init__(self, model_name, op, load_dic, filename_options=[], dp_params={}):
        ...
        ## if self.loaded_from_checkpoint: will load the data from the file
        ## else init a blank model_state and prepared to save the data
...
#            print self.save_file
        ...
        if load_dic: 
            self.train_data_provider.advance_batch()            ## init data provider test_data_provider or train_data_provider
            
        # model state often requries knowledge of data provider, so it's initialized after
        try:
            ## init the model state, include traing_output, epoch, batchnum, test_outputs, layers list(get from LayersParser) and so on.
    ...
    def fill_excused_options(self):
        pass
    ###############################################################################################################
    ## init the test dataProvider and
    ##      the train dataProvider
    ##
    ...
    def train(self):
        ...
        if self.save_path in self.save_file:
            print self.save_file
        else:
            save_file_name = "ConvNet_%s" %(strftime("%Y-%m-%d_%H.%M.%S", localtime()))
            # self.save_file = os.path.join(self.save_path,save_file_name)
            self.save_file = self.save_path + '/ConvNet_' + str(self.train_batch_range[0])
    ...
    def get_next_batch(self, train=True):
        ...
        # dp.get_next_batch(): test or train DataProvider.get_next_batch()
        temp = dp.get_next_batch()
        return self.parse_batch_data(temp, train=train)
    ## what batch_data[0], [1] and [2]
    ...
    ## run by c++ finishBatch and get return results
    ...
    ## save the processing state to the file: os.path.join(self.save_file, checkpoint_file)
    def save_state(self):
    ...
    ##############################################################
    ## static method: read file contend
    ## Read by xyongle on 2015-11-01
    ##
    @staticmethod
